    ...
    <x-header>
        ...
        <div class=" h-full w-full overflow-hidden overflow-y-scroll custom-scrollbar">
            <div class="flex items-center justify-start gap-4">
                <h3 class="text-2xl min-w-12 max-w-40 overflow-x-scroll text-nowrap custom-scrollbar">{{ $playlist->name }}</h3>
                <a class="hover:underline transition-all ease-in-out" href="/compose/playlist/{{ $playlist->id }}/songs" draggable="false"><h3 class="text-2xl text-gray-400 min-w-12 max-w-40 overflow-x-scroll text-nowrap custom-scrollbar" draggable="false">{{ count($playlist->songs) }} songs</h3></a>
                <a class="hover:underline transition-all ease-in-out" href="/compose/users/{{ $playlist->user_id }}/show" draggable="false"><h3 class="text-2xl text-red-500 min-w-12 max-w-36 overflow-x-scroll text-nowrap  custom-scrollbar" draggable="false">Created By {{ $playlist->user->name }}</h3></a>
                ...
                <x-forms.form action="/compose/playlists/{{ $playlist->id }}/destroy" id="deleteplaylistform" method="DELETE">
                    <x-forms.form-iconbtn form="deleteplaylistform" src="delete.png"/>
                </x-forms.form>
                @endif
            </div>
