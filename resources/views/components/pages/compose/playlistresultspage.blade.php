...
<div class="border-b-2 w-full bg-color1 p-2 rounded-t-2xl flex gap-2 overflow-x-scroll custom-scrollbar">
    @if(count($playlists) > 0)
        @foreach ($playlists as $playlist)
            <x-walkman.resultlink image="" src="{{ asset('images/redplaylist.png') }}" href="/compose/playlists/{{ $playlist->id }}/show" name="{{ $playlist->name }}" type="{{ $playlist->user->name }} playlist"/>
        @endforeach
    @else
